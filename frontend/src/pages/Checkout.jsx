  ...
  return (
    <div className="shop-page checkout-page">
      ...
      <main className="checkout-layout">
        <section><p className="eyebrow">Secure checkout</p><h1>Finish your<br /><em>selection.</em></h1><form onSubmit={async (event) => { event.preventDefault(); setPaymentMessage(""); const total = subtotal >= 100 ? subtotal : subtotal + 8; try { if (paymentProvider !== "offline") await api.createPayment({ provider: paymentProvider, amount: total, currency: "usd" }); const number = `GDS-${Date.now().toString().slice(-6)}`; const nextOrder = { id: number, items: cart, total, status: "Processing", paymentProvider, createdAt: new Date().toISOString() }; saveOrders([nextOrder, ...loadOrders()]); saveCart([]); setOrderNumber(number); setComplete(true); } catch (error) { setPaymentMessage(error.message); } }}>
          ...
          <div className="checkout-block"><h2>Payment</h2><div className="payment-options"><label><input type="radio" name="provider" value="offline" checked={paymentProvider === "offline"} onChange={(event) => setPaymentProvider(event.target.value)} /> Pay on confirmation</label><label><input type="radio" name="provider" value="stripe" checked={paymentProvider === "stripe"} onChange={(event) => setPaymentProvider(event.target.value)} /> Stripe (configured stores only)</label><label><input type="radio" name="provider" value="paypal" checked={paymentProvider === "paypal"} onChange={(event) => setPaymentProvider(event.target.value)} /> PayPal (configured stores only)</label></div>{paymentProvider === "offline" && <><input required placeholder="Card number" inputMode="numeric" /><div className="form-grid"><input required placeholder="MM / YY" /><input required placeholder="CVC" /></div></>}{paymentMessage && <p className="dashboard-message">{paymentMessage}</p>}</div>
